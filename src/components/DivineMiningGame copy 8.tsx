...
export const DivineMiningGame: React.FC = () => {
  ...
  // Real-time points display for upgrade modal - update immediately when modal opens
  const [modalPoints, setModalPoints] = useState(gameState.divinePoints);
  
  useEffect(() => {
    // Update points immediately when modal opens
    if (showUpgradeShop) {
      setModalPoints(gameState.divinePoints);
    }
  }, [showUpgradeShop, gameState.divinePoints]);
  
  // Also update points every 2 seconds while modal is open to keep it current
  useEffect(() => {
    if (!showUpgradeShop) return;
  ...
  // Upgrade modal content - removed memoization to ensure real-time updates
            ...
            <div className="flex items-center space-x-2">
              {/* <div className="text-right">
                <div className="text-sm font-mono font-bold text-cyan-300">
                  {formatNumber(modalPoints)}
                </div>
                <div className="text-xs font-mono text-cyan-400">TBC</div>
              </div> */}
              <button
  ...
  return (
    <>
      ...
      <div className="flex-1 flex flex-col items-center justify-center space-y-4 overflow-y-auto game-scrollbar">
          ...
          {showUpgradeShop && upgradeModalContent()}
