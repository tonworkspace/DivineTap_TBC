      ...
      <div className="bg-black/40 backdrop-blur-xl border border-cyan-500/30 rounded-xl p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-cyan-300 text-lg font-bold">5 TBC</div>
            <div className="text-gray-400 text-xs mt-1">Daily reward</div>
          </div>
        </div>
        <div className="mt-3 pt-3 border-t border-cyan-500/20">
          <div className="text-gray-400 text-xs">
            • Earn 5 TBC daily for each active referral
            <br />
            • Bonus 10 TBC when referral reaches level 10
            <br />
            • Extra 2 TBC from sub-referrals
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          <button 
            onClick={() => {
              ...
              const url = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(`https://t.me/DivineTaps_bot/mine?startapp=${referralData.code}`)}`;
